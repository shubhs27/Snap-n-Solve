# This file detects a Sudoku board, extracts and recognize digits, solve the puzzle and print the result back to the video frame
...
def write_solution_on_image(image, grid, user_grid, difficulty=None):
    ...
    # Add the grid lines for better visualization
    for i in range(1, 9):
        line_thickness = 2 if i % 3 == 0 else 1
        # Horizontal lines
        cv2.line(image, (0, i*height), (image.shape[1], i*height), (0, 0, 0), line_thickness)
        # Vertical lines
        cv2.line(image, (i*width, 0), (i*width, image.shape[0]), (0, 0, 0), line_thickness)
    
    # Draw thicker outer border
    cv2.rectangle(image, (0, 0), (image.shape[1]-1, image.shape[0]-1), (0, 0, 0), 3)
    

    for i in range(SIZE):
        for j in range(SIZE):
            if user_grid[i][j] != 0:    # If user filled this cell
                continue                # Move on
            
            # Write the solution number directly, without the semi-transparent overlay
            text = str(grid[i][j])
            off_set_x = width // 15
            off_set_y = height // 15
            ...
            font_scale = 0.7 * min(width, height) / max(text_height, text_width)  # Slightly bigger font
            ...
            image = cv2.putText(image, text, (bottom_left_corner_x, bottom_left_corner_y), 
                                font, font_scale, (0, 0, 0), thickness=5, lineType=cv2.LINE_AA)
            image = cv2.putText(image, text, (bottom_left_corner_x, bottom_left_corner_y), 
...
def recognize_and_solve_sudoku(image, model, old_sudoku):
    ...
    if biggest_contour is None:        # If no sudoku
        # Add status message to the image
        cv2.putText(image, "No Sudoku detected", (20, image.shape[0] - 40), 
    ...
    if corners is None:         # If no sudoku
        # Add status message to the image
        cv2.putText(image, "No valid Sudoku board detected", (20, image.shape[0] - 40), 
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        return image
    ...
    and approx_90_degrees(angle_between(BC,DC), eps_angle) and approx_90_degrees(angle_between(AD,DC), eps_angle)):
        # Add status message to the image
        cv2.putText(image, "Not a valid Sudoku board shape", (20, image.shape[0] - 40), 
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        return image
    ...
    if(side_lengths_are_too_different(A, B, C, D, eps_scale)):
        # Add status message to the image
        cv2.putText(image, "Sudoku board is too skewed", (20, image.shape[0] - 40), 
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        return image
    ...
    difficulty, score = sudokuDifficulty.calculate_difficulty(grid)
